...
% not quite done here. need more on specific types of ZRP (Hopnet in particular. Set up for proposed alg)
% next: scalability literature with zones and how it has not yet been used for traffic assignment

%Seperately, the recent literature also includes approaches for scaling the effectiveness of algorithms which cover large graphs. Much of the work in this section is derived from the study of networking. Proactive (routes stored in advance) and reactive (routes evaluated on-demand) routing strategies are both examined. Hybird strategies, where proactive and reactive routing is used in the same system, are also discussed. The hybrid routing strategy of note is the zone routing protocol (ZRP)
