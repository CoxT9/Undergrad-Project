...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
  \Procedure{SZDiscovery}{$Vehicle, G, AllZones$}
  \State{$src \gets Vehicle.nextNode$}
  \State{$dest \gets Vehicle.destination$}
  \State{$exit\ unless\ src \neq dest$}
  \State{$zones \gets AllZones.reduce(z: src \in z \wedge dest \in z)$}
  \State{$paths \gets zones.map(z: z.table[src,dest])$}
  \State{$Vehicle.route \gets min(paths)$}
  \EndProcedure
  \end{algorithmic}
\end{algorithm}

% Give a quick description.
In order for single-zone discovery to be successful, two assumptions are required. First, the vehicle's source and destination nodes must be distinct. Second, the two nodes must be present in the same zone. If the vehicle's source \textit{is} its destination, no work is required and the procedure exits. Otherwise, all zones containing both the source and destination nodes are gathered and mapped to their corresponding \textit{src, dest} table entries. The minimum of these routes is assigned to the vehicle. If no nodes containing the source zone also contain the destination zone, Z-BAR moves on to the multi-zone discovery phase.

\subsubsection{Multi-Zone Route Discovery}

A vehicle only engages in multi-zone route discovery when no zones which include its source also include its destination.

% Write up multizone discovery. Use this as a chance to tweak and improve the alg itself
