...
\begin{algorithm}[H]
  ...
  \State{$zones \gets AllZones.filter(z: src \in z \wedge dest \in z)$}
  \State{$paths \gets zones.map(z: z.table[src,dest])$}
  \State{$Vehicle.route \gets min(min(paths), Vehicle.route)$}
  \EndProcedure
  \end{algorithmic}
\end{algorithm}
...
% Very first draft:
% This will need some editing in tandem with writing out the procedure
% Big question, is it the vehicles or the zones that do the searching. Likely the zones - vehicles do not have topology awareness
