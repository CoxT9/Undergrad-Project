...
def parseArgs():
  ...
  return config, gui, csvFile, verbose
...
def launchSim(zoneStore, nodeToZoneDict, network, sim, config, verbose):
  ...
  for step in xrange(int(config.endtime)):
    for v in traci.vehicle.getIDList():
      if traci.vehicle.getSpeed(v) <= 0.1: # Effectively stopped
        try:
          waitTimes[v] += 1
        except KeyError:
          waitTimes[v] = 1
...
def averages(logfile, agents, waitTimes):
  ...
  avgWT = 0
  for a in logfile:
    time = float(a.arrival) - float(a.depart)
    wait = waitTimes[a.id]

    avgTT += time
    avgWT += wait

  return (avgTT/totalCount, avgWT/totalCount)
...
def main():
  starttime = time.time()
  config, gui, csvFile, verbose = parseArgs()
